...
module.exports = {
  addPlace: async (req, res, next) => {
    ...
    try {
      const newPlace = new Place({
        country_id, description, imageUrl, location, title, rating, review, latitude, longitude
      })

      await newPlace.save()

      res.status(201).json({ status: true, message: 'Place saved successfully' })
    } catch (error) {
      return next(error)
    }
  },
  getPlaces: async (req, res, next) => {
    try {
      const places = await Place.find({}, '__id review rating imageUrl country_id')
      .populate({
        path: 'popular',
